...
} //Copies CString array to a vector<char>
...
inline void removeSpaceChar(std::vector<char> &vector) {
    std::remove(vector.begin(), vector.end(), ' ');
} //Removes instances of the space character from the vector.

inline void removeAposChar(std::vector<char> &vector) {
    std::remove(vector.begin(), vector.end(), '\'');
} //Removes any instances of the apostrophe from the vector. Passes vector address to the function.

inline void sortVector(std::vector<char> &vector) {
    std::sort(vector.begin(), vector.end());
} //Sorts vector in ascending order.

inline void uniqueCopyTo(std::vector<char> &vector, std::vector<char> &copy) {
...
inline bool isInVector(std::vector<char> &vector, char c) {
